            ...
            switch (opção){
                ...
                case "A00":
                    String a00 = String.format("A00%-50s%10s%15s%n",
                            pacient.getName(), pacient.getBirthday(), pacient.getCellphone());
                    saida.format("%s", a00.replaceAll("null", ""));
                    break;
                case "C00":
                    for (int i=0; i < lista.getTamanho(); i++) {
                        MedicalRecord ficha = (MedicalRecord)lista.getElemento(i);
                        String c00 = String.format("C00%02d%04.1f%04.1f%04.1f%04.1f%1s%-50s%-8s%-10s%-8s%-8s%-20s%n",
                                ficha.getId(),
                                ficha.getWeight(),
                                ficha.getHeight(),
                                ficha.getBloodPressure(),
                                ficha.getBodyTemperature(),
                                ficha.getAllergy(),
                                ficha.getLastCycle(),
                                ficha.isPregnant(),
                                ficha.getCreatedAt(),
